...
use std::env;
use std::fs::{create_dir, read_dir, remove_dir_all, File, OpenOptions};
...
impl Command {
    pub fn new(input: Vec<u8>) -> std::io::Result<Self> {
        ...
        let command = match command_lowercase.as_slice() {
            ...
            b"type" => Command::Type,
            b"list" => Command::List(
                data.map(|bytes| PathBuf::from(String::from_utf8_lossy(bytes).to_string())),
            ),
            b"pasv" => Command::Pasv,
            b"cwd" => Command::Cwd(
                data.map(|bytes| PathBuf::from(String::from_utf8_lossy(bytes).to_string()))
...
impl Client {
    ...
    fn stor(&mut self, path: PathBuf) {
        ...
        if let Ok(file_path) = self.complete_path(path, &server_root) {
            send_cmd(
                &mut self.stream,
                ResultCode::OpeningDataConnection,
        ...
        if let Ok(file_path) = self.complete_path(path, &server_root) {
            if let Ok(mut file) = File::open(file_path) {
                send_cmd(
                    &mut self.stream,
                    ResultCode::OpeningDataConnection,
                    "Opening binary mode data connection for file download.",
                );
                if let Some(ref mut writer) = self.data_writer {
                    let mut buffer = [0u8; 1024];
                    loop {
